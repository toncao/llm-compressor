...
class Step3p5ExpertMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Store weights as float32 so nn.Linear forward + hooks work
        self.up_proj.weight.data = self.up_proj.weight.data.float()
        self.gate_proj.weight.data = self.gate_proj.weight.data.float()
        self.down_proj.weight.data = self.down_proj.weight.data.float()
        ...
        out = self.down_proj((gate * up).float())
    ...
    def __init__(self, config: "Step3p5Config", original_moe: "Step3p5MoEMLP"):
        ...
        for i in range(num_experts):
            self[i].gate_proj.weight.data = (
                original_moe.gate_proj.weight[i].clone().contiguous()
            )
            self[i].up_proj.weight.data = (
                original_moe.up_proj.weight[i].clone().contiguous()
            )
            self[i].down_proj.weight.data = (
                original_moe.down_proj.weight[i].clone().contiguous()
            )
